"""
1小时 Swing 二分类回测
  入场：确认 TROUGH 后下一根 1h bar
  出场：确认 PEAK 后下一根 1h bar
  趋势过滤：价格 > 13根1h MA（≈2交易日均线）
  止损：默认2%

python backtest_swing_1h.py
python backtest_swing_1h.py --confidence 0.60 --symbols AAPL NVDA AMD
"""
import argparse, os
...
from train_swing_1h import load_1h, THRESHOLDS_1H, ZIGZAG_THRESH, CACHE_DIR_30M
...
def backtest_single(df: pd.DataFrame, clf, symbol: str,
    ...
    for pi, pivot in enumerate(pivots):
        ...
        if position is not None:
            ...
            if cur <= position["stop_price"]:
                slip = cur * (1 - slippage)
                net  = position["shares"] * slip * (1 - commission)
    ...
    if position and n > 0:
        ...
        capital += net
        trades.append({"symbol": symbol, "pnl_pct": (net-position["cost"])/position["cost"],
...
def main():
    ...
    print("[1/3] 加载模型...")
    clf = joblib.load("saved_models/swing_1h_gbm.pkl")
    ...
    print(f"  {'PORT':<6} {port_m['total_return']:>+8.1f}%  {port_bnh:>+8.1f}%  "
          ...
          f"{port_m['max_drawdown']:>6.1f}%  {port_m['win_rate'] or 0:>5.0f}%  "
    ...
    if all_trades:
        ...
        for lo, hi in [(0.50,0.55),(0.55,0.60),(0.60,0.65),(0.65,0.70),(0.70,1.0)]:
            ...
            if sub:
                ...
                stop_cnt = sum(1 for t in sub if t.get("reason") == "stop_loss")
                print(f"    [{lo:.2f},{hi:.2f}): n={len(sub):4d}  "
                      f"winrate={wr*100:.0f}%  avg={avg:+.3f}%  "
                      f"stop_loss={stop_cnt}")
    ...
    axes[0].set_title(f"1h Swing conf={args.confidence} stop={args.stop_loss*100:.1f}%")
    ...
    plt.tight_layout()
    path = "results/swing_1h_backtest.png"
